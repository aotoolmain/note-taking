    ...
    groupedNotes.forEach(([dateLabel, groupNotes], index) => {
        ...
        groupNotes.forEach(note => {
            ...
            if (currentId === note.id) {
                ...
                item.onmouseout = function() {
                    if (this.classList.contains('active')) {
                        this.style.backgroundColor = 'var(--bg-card-active, var(--bg-card-hover))';
            ...
            editBtn.onclick = async (e) => {
                ...
                const handleBlur = () => { handleEdit(true); };
...
async function loadCate() {
    ...
    categories.forEach(cat => {
        ...
        d.onmouseout = function() {
            if (!this.classList.contains('active')) this.style.backgroundColor = 'transparent';
        };
        ...
        nameDiv.onclick = async () => { selectCategory(cat.id); };
        ...
        if (!cat.isDefault) {
            ...
            editBtn.onclick = async (e) => {
                ...
                const handleKeydown = (e) => {
                    if (e.key === 'Enter') { e.preventDefault(); handleEdit(true); }
                    else if (e.key === 'Escape') handleEdit(false);
                };
                const handleBlur = () => { handleEdit(true); };
            ...
            deleteBtn.onclick = async (e) => {
                ...
                const data = await res.json();
                if (data.success) {
                    if (currentCateId === String(cat.id)) selectCategory('all');
                    await loadCate();
                    await loadNotes();
                }
            };
